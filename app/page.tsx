...
export default function PhotoBoothApp() {
  ...
  const [selectedStripDesign, setSelectedStripDesign] =
    useState<StripDesign>("college");
  ...
  const availableFilters: {
    ...
    },
    {
      name: "Vintage",
      label: "Vintage",
      css: "sepia(0.5) contrast(1.1) saturate(1.2) brightness(1.1)",
      icon: "📸",
    },
    {
      name: "Polaroid",
      label: "Polaroid",
      css: "saturate(1.2) contrast(1.05) brightness(1.1) sepia(0.1)",
      icon: "📷",
    },
    {
      name: "Warm",
      label: "Warm",
      css: "sepia(0.2) saturate(1.3) hue-rotate(20deg) brightness(1.1)",
      icon: "🔥",
    },
    {
      name: "Cool",
      label: "Cool",
      css: "sepia(0.1) saturate(1.2) hue-rotate(200deg) brightness(1.0)",
      icon: "❄️",
    },
    {
      name: "Dramatic",
      label: "Dramatic",
      css: "contrast(1.5) saturate(1.1) brightness(0.9)",
      icon: "🎬",
    },
    {
      name: "Bright",
      label: "Bright",
      css: "saturate(1.4) contrast(1.1) brightness(1.3)",
      icon: "☀️",
    },
    {
      name: "Retro",
      label: "Retro",
      css: "sepia(0.3) saturate(1.5) hue-rotate(350deg) contrast(1.2)",
      icon: "📺",
    },
  ];
  ...
  const createPhotoStrip = useCallback(() => {
    ...
    const template =
      stripTemplates.find((t) => t.name === selectedStripDesign) ||
      stripTemplates[0];
    ...
    if (template.name === "college") {
      ...
      for (let i = 0; i < 20; i++) {
        ctx.beginPath();
        ctx.arc(
          Math.random() * STRIP_WIDTH,
          Math.random() * STRIP_HEIGHT,
          2,
          0,
          Math.PI * 2
        );
    ...
    capturedPhotos.forEach((photo, index) => {
      ...
      img.onload = () => {
        ...
        const filterUsed = availableFilters.find(
          (f) => f.name === photo.filter
        );
        if (filterUsed) {
          ctx.save();
          ...
          ctx.textAlign = "right";
          ctx.fillText(
            `${filterUsed.icon} ${filterUsed.label}`,
            MARGIN + PHOTO_WIDTH - 5,
            yPosition + 25
          );
        }
        ...
        <div className="flex items-center justify-center min-h-screen relative">
          <div className="text-center z-20">
            ...
            <div className="mt-8 text-white text-lg font-medium">
              Perfect for dorm rooms, graduation parties, and friendship
              memories! 📱✨
            </div>
          </div>
          ...
          <div className="absolute inset-0 z-10 pointer-events-none">
            <div
              ...
              style={{
                background:
                  "repeating-linear-gradient(90deg, #991b1b 0px, #dc2626 20px, #b91c1c 40px)",
                boxShadow:
                  "inset -20px 0 40px rgba(0,0,0,0.3), 20px 0 40px rgba(0,0,0,0.5)",
              }}
            >
            ...
            <div
              ...
              style={{
                background:
                  "repeating-linear-gradient(270deg, #991b1b 0px, #dc2626 20px, #b91c1c 40px)",
                boxShadow:
                  "inset 20px 0 40px rgba(0,0,0,0.3), -20px 0 40px rgba(0,0,0,0.5)",
              }}
            >
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-200 via-pink-200 to-orange-300 relative overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-20 flex gap-3">
        <Button
          ...
          className="group relative bg-rose-500 hover:bg-rose-500/20 active:bg-white/30 text-white px-5 py-2.5 rounded-xl font-medium shadow-lg backdrop-blur-md border border-white/20 hover:border-white/30 transition-all duration-200 ease-in-out hover:scale-105 hover:shadow-xl"
        >
        ...
        >
          <span className="flex items-center gap-2">
            <span className="text-lg">✏️</span>
            <span className="text-sm font-semibold tracking-wide">Names</span>
          </span>
        </Button>
      </div>
      ...
      {/* Settings Panel */}
      {showSettings && (
        <div className="absolute top-20 right-4 z-30 bg-white rounded-2xl p-6 shadow-2xl border border-gray-200 max-w-md w-full">
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            <p className="text-xs text-gray-500 text-center">
              Choose a template to customize your photo strip design
            </p>
          </div>
        </div>
      )}
            ...
            <div className="relative">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {availableFilters.map((filter) => (
                    <Button
                      key={filter.name}
                      variant={
                        currentFilter === filter.name ? "default" : "ghost"
                      }
                      size="sm"
                ...
                <div className="mt-6">
                  ...
                  <div className="flex gap-2 justify-center">
                    {capturedPhotos.map((photo, index) => (
                      <div
                        key={index}
                        className="w-16 h-12 rounded-lg overflow-hidden shadow-md"
                      >
                        <img
                          src={photo.dataUrl}
                          alt={`Photo ${index + 1}`}
              ...
              <div className="text-center mt-4">
                <p className="text-white text-sm font-medium">
                  {capturedPhotos.length === 0 &&
                    "📸 Click to take your first photo"}
                  {capturedPhotos.length === 1 &&
                    "🎉 Great! Take 2 more photos"}
                  {capturedPhotos.length === 2 && "🔥 One more photo to go!"}
                  {capturedPhotos.length === 3 &&
                    "✨ All photos taken! Creating your strip..."}
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 rounded-3xl p-8 max-w-4xl w-full shadow-2xl border-0">
            <div className="text-center mb-6">
              <h2 className="text-3xl font-bold text-white mb-2">
                🎓 Your College Memory Strip!
              </h2>
              <p className="text-purple-200">
                Perfect for sharing with friends and keeping forever
              </p>
            </div>
            ...
            <div className="flex gap-8 items-center justify-center">
              <div className="bg-white p-6 rounded-2xl shadow-2xl transform rotate-1 hover:rotate-0 transition-transform duration-300">
                <canvas
                  ...
                  className="max-w-[280px] w-full h-auto rounded-lg shadow-inner"
                />
              </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-white bg-opacity-10 rounded-xl p-4 backdrop-blur-sm">
                  ...
                  {userName && (
                    <p className="text-purple-200">Made by: {userName}</p>
                  )}
                  {groupName && (
                    <p className="text-purple-200">Event: {groupName}</p>
                  )}
                </div>
            ...
            <div className="mt-8 text-center">
              <p className="text-purple-200 text-sm">
                Share your college memories with friends! Tag @pawan_dasila in
                your social posts 📱
              </p>
            </div>
          </Card>
